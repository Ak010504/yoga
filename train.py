"""
training pipeline for PosePilot LSTM model using the dataset.
this script processes individual CSV files from the data/ directory structure.
"""
...
def process_csv_to_sequence(df, pose, person, camera):
    ...
    try:
        ...
        selected_data = selected_top_frames(data_with_error, 10)
...
def main():
    ...
    parser.add_argument(
        "--test_size", type=float, default=0.2, help="Test set size fraction"
    )
    ...
    processed_cache_path = os.path.join(data_dir, "cached_features.pkl")
